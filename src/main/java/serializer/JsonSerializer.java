...
public class JsonSerializer implements Serializer {
    ...
    public String generateResult(List<TraceResult> traceResults) {
        ...
        for (TraceResult traceResult : traceResults) {
            ...
            writer.append("\t{\n");
            writer.append("\t\t\"threadId\": \"").append(traceResult.getThreadId()).append("\",\n");
            writer.append("\t\t\"time\": \"").append(traceResult.getNanoseconds()).append("ns\",\n");
    ...
    private void appendMethods(int tabs, StringBuilder writer, List<CustomMethod> methods) {
        ...
        for (CustomMethod method : methods) {
            writer.append(tabForTitle).append("{\n");
            writer.append(tab).append("\"name\": \"").append(method.getName()).append("\",\n");
            writer.append(tab).append("\"class\": \"").append(method.getClassName()).append("\",\n");
            writer.append(tab).append("\"time\": \"").append(method.getNanoseconds()).append("ns\",\n");
            appendMethods(tabs + 2, writer, method.getCustomMethods());
            writer.append(tabForTitle).append("}").append(delimeter(method, methods)).append("\n");
        }
        writer.append(tabForTitle).append("]\n");
    }
